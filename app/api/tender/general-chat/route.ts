...
`.trim();
      ...
      try {
        payload = await requestOpenAiResponse({
          ...
          timeoutMs: files.length > 0 ? 18000 : 40000,
        });
      } catch {
...
`.trim();
        ...
        try {
          payload = await requestOpenAiResponse({
            ...
            inputFiles: useDirectFileMode ? [] : uploadedCurrentFiles,
            useWebSearch: false,
            reasoningEffort: "low",
            timeoutMs: 12000,
          });
      ...
      const answer =
        ...
        [
          ...
          effectiveFileReadStates.length > 0 ? "Что уже вижу:" : "",
          ...effectiveFileReadStates.slice(0, 5).map((item) => `- ${item}`),

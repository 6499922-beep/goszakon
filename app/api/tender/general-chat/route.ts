...
export async function POST(request: Request) {
  try {
    ...
    const fileAnalysisInputs = limitedFileBlocks.slice(0, 4);
    const fileAnalyses = await Promise.all(
      fileAnalysisInputs.map(async (block) => {
        const filePrompt = `
...
`.trim();
        ...
        try {
          ...
          return `Файл удалось прочитать не полностью. Черновой контекст: ${trimForPrompt(block, 700)}`;
        }
      })
    );
...
Дай короткий и полезный ответ по вопросу пользователя, опираясь только на историю и краткие выжимки по файлам.
...
Выжимки по файлам:
${fileAnalyses.length > 0 ? fileAnalyses.map((item, index) => `${index + 1}. ${item}`).join("\n") : "Файлы не приложены."}

...
import {
  getThematicPracticePage,
  THEMATIC_PRACTICE_PAGES,
} from "@/lib/thematic-practice-pages";
...
export async function generateStaticParams() {
  return THEMATIC_PRACTICE_PAGES.map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata({
  ...
  const page = getThematicPracticePage(slug);
...
export default async function ThematicPracticePage({ params }: PageProps) {
  const { slug } = await params;
  const page = getThematicPracticePage(slug);
  ...
  const siblings = THEMATIC_PRACTICE_PAGES.filter((item) => item.slug !== slug).slice(
    0,
    3,
  );
          ...
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            <div className="rounded-3xl bg-[#081a4b] p-8 text-white shadow-sm">
              ...
              <ul className="mt-5 space-y-4">
                {page.whatToCheckFirst.map((item) => (
                  <li key={item} className="rounded-2xl bg-white/10 p-4 text-base leading-8 text-white/90">
                    {item}
                  </li>
                ))}
              </ul>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-3xl border border-emerald-200 bg-emerald-50 p-8">
              ...
              <div className="mt-5 space-y-4">
                {page.whatUsuallyWorks.map((item) => (
                  <div
                    key={item}
            ...
            <div className="rounded-3xl border border-rose-200 bg-rose-50 p-8">
              ...
              <div className="mt-5 space-y-4">
                {page.whatUsuallyBreaks.map((item) => (
                  <div
                    key={item}
                    className="rounded-2xl border border-rose-200 bg-white p-5 text-base leading-8 text-slate-700"
                  >
                    {item}
                  </div>
                ))}

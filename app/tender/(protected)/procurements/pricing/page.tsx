...
function getPricingStatusMeta(status: string | null, comment: string | null) {
  switch (status) {
    case "profitable":
      return {
        label: "Цены найдены",
        tone: "bg-emerald-50 text-emerald-700",
        note: "Просчёт выполнен.",
      };
    case "low_margin":
      return {
        label: "Низкая маржа",
        tone: "bg-rose-50 text-rose-700",
        note: comment?.trim() || "Найдена низкая рентабельность.",
      };
    case "not_found":
      return {
        label: "Цены не найдены",
        tone: "bg-amber-50 text-amber-700",
        note: comment?.trim() || "Нужен дополнительный поиск цены.",
      };
    case "manual_review":
      return {
        label: "Нужна проверка",
        tone: "bg-amber-50 text-amber-700",
        note: comment?.trim() || "По заявке нужен ручной просчёт.",
      };
    default:
      return {
        label: "На просчёте",
        tone: "bg-blue-50 text-blue-700",
        note: "Заявка передана на второй этап.",
      };
  }
}

function getPricingStatusNote(note: string) {
...
export default async function TenderPricingPage() {
  ...
  const procurements = await prisma.tenderProcurement.findMany({
    where: { status: TenderProcurementStatus.PRICING },
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-2 text-sm text-slate-500">
              Здесь только заявки, которые переданы из анализа на второй этап.
            </div>
          </div>
              ...
              {procurements.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-10 text-center text-slate-500">
                    Пока нет заявок, переданных на просчёт.
                  </td>
                </tr>
              ) : (
                procurements.map((item) => {
                  ...
                  const registryRecord = normalizedInn
                    ...
                    : null;
                  const pricingMeta = getPricingStatusMeta(
                    item.pricingStatus,
                    item.pricingComment
                  );
                  const stopTone =
                  ...
                  const stopLabel = item.stopFactorsSummary?.toLowerCase().includes("не выявлены")
                    ? "Не выявлены"
                    : "Проверить";
                  const rowHref = `/procurements/pricing/${item.id}`;
                      ...
                      <td className="px-4 py-4">
                        <a href={rowHref} className="block -mx-4 -my-4 px-4 py-4">
                          ...
                          <div className="mt-2 max-w-xs text-xs leading-5 text-slate-500">
                            {getPricingStatusNote(pricingMeta.note) ?? "Открыть карточку заявки"}
                          </div>
                        </a>
                      </td>
                      <td className="px-4 py-4">

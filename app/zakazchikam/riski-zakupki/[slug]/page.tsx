...
export async function generateStaticParams() {
  return CUSTOMER_RISK_PAGES.map((item) => ({ slug: item.slug }));
}
...
export async function generateMetadata({
  ...
  const page = getCustomerRiskPage(slug);
...
export default async function CustomerRiskDetailPage({ params }: PageProps) {
  const { slug } = await params;
  const page = getCustomerRiskPage(slug);

  if (!page) notFound();

  const siblings = CUSTOMER_RISK_PAGES.filter((item) => item.slug !== slug).slice(
    0,
    3,
  );
        ...
        <div className="mx-auto max-w-7xl px-6 py-20">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-5 text-4xl font-bold tracking-tight text-[#081a4b] md:text-5xl">
              Связанные направления по этой теме
            </h2>
          </div>
        ...
        <div className="mx-auto max-w-7xl px-6 py-20">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {siblings.map((item) => (
              <Link
                key={item.slug}
                href={`/zakazchikam/riski-zakupki/${item.slug}`}

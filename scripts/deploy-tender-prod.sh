...
ssh -i ~/.ssh/id_ed25519_goszakon -o StrictHostKeyChecking=no "root@${PROD_HOST}" "chmod +x ${PROD_DEPLOY_SCRIPT} && APP_PATH=${PROD_PATH} COMPOSE_FILE=docker-compose.tender.yml APP_SERVICE=app APP_CONTAINER=goszakon-tender-app REMOVE_ORPHANS=false ${PROD_DEPLOY_SCRIPT}"
